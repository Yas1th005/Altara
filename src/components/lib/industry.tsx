...
export default function AboutUsSection() {
  ...
  return (
    <section
      id="about-section"
      ref={sectionRef}
      className="w-full py-24 px-4 bg-black from-[#F2F2EB] to-[#F8F8F2] text-[#202e44] overflow-hidden relative"
    >
...
function ServiceItem({ icon, secondaryIcon, title, description, variants, delay, direction }: ServiceItemProps) {
  return (
    ...
    >
      ...
      >
        <motion.div
          className="text-[#ff774e] bg-[#88734C]/10 p-3 rounded-lg transition-colors duration-300 group-hover:bg-[#88734C]/20 relative"
          whileHover={{ rotate: [0, -10, 10, -5, 0], transition: { duration: 0.5 } }}
        >
          {icon}
          {secondaryIcon}
        </motion.div>
        <h3 className="text-xl font-medium bg-gradient-to-r from-orange-300 from-20% to-white bg-clip-text text-transparent group-hover:text-[#88734C] transition-colors duration-300">
  {title}
</h3>
      </motion.div>
      <motion.p
